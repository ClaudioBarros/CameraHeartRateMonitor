...
public abstract class CameraBridgeViewBase extends SurfaceView implements SurfaceHolder.Callback {
    ...
    /**
     * Functions below are part of a fix for the landscape camera issue
     */
    private void updateMatrix(){
        float matrixWidth = this.getWidth();
        float matrixHeight = this.getHeight();
        float halfWidth = matrixWidth/2.0f;
        float halfHeight = matrixHeight/2.0f;

        float displayWidth = (float)Resources.getSystem().getDisplayMetrics().widthPixels;
        float displayHeight = (float)Resources.getSystem().getDisplayMetrics().heightPixels;

        float scaleWH = displayWidth / matrixHeight;
        float scaleHW = displayHeight / matrixWidth;
        float scaleFactor = scaleWH;
        if(scaleHW > scaleWH) {
           scaleFactor = scaleHW;
        }
        ...
        if(isFrontCamera){
            mMatrix.preScale(-1, 1, halfWidth, halfHeight); //Mirror Camera
        }
        ...
        mMatrix.preTranslate(-halfWidth, -halfHeight);
        mMatrix.preScale(scaleFactor, scaleFactor, halfWidth, halfHeight);
